...
export const AdminPage: React.FC = () => {
  ...
  // Subscribe to Supabase Auth session & auth state changes
  useEffect(() => {
    ...
    try {
      ...
      subscriptionObj = res?.data?.subscription;
    } catch (err) {
  ...
  // Handle Login via Supabase Auth email and password
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured()) {
      setLoginError('Supabase Anon Key is missing. Please set VITE_SUPABASE_ANON_KEY in your Vercel Environment Variables.');
      setAuthLoading(false);
      return;
    }
