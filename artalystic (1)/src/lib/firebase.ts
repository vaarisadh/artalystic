...
/** Upload file (JPG, JPEG, PNG, WEBP, PDF) to Firebase Storage */
export async function uploadArtworkFile(
  ...
  try {
    ...
    const fileRef = ref(storage, `${folder}/${timestamp}_${sanitizedName}`);
    const task = await uploadBytesResumable(fileRef, file);
    const url = await getDownloadURL(task.ref);
    return { url, fileType: file.type || 'image/png', error: null };
  } catch (err: any) {
    console.error('Firebase Storage upload error:', err);
    return { url: '', fileType: '', error: err.message || 'File upload failed.' };
  }
}
